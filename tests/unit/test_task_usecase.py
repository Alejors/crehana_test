...
async def test_get_task(
    mock_task_repo, mock_task_list_repo, mock_user_repo, mock_mailing_service
):
...
async def test_create_task_with_assignee(
    ...
    mock_task_repo.create = AsyncMock(return_value=TASK1)
    mock_user_repo.get_user_by_email = AsyncMock(
        return_value=User(id=1, email="test@test.com", password="abc123")
    )
    ...
    task_in = Task(
        description="test-1", task_list_id=1, assigned_user_email="test@test.com"
    )
...
@pytest.mark.asyncio
async def test_create_task_with_inexistent_assignee(
    mock_task_repo, mock_task_list_repo, mock_user_repo, mock_mailing_service
):
    mock_task_repo.create = AsyncMock(return_value=TASK1)
...
async def test_update_none(
    ...
    # (AKA: llena de None's)
    task_in = Task(
        description=None, task_list_id=None, is_completed=None, priority=None
    )
