...
def create_task_lists_route(task_list_usecase: TaskListUsecase) -> APIRouter:
    ...
    async def get_task_lists() -> List[TaskListOut]:
        ...
        return [TaskListOut.from_entity(task_list) for task_list in task_lists if task_list]
    ...
    @router.post("/task-lists", status_code=status.HTTP_201_CREATED) #TODO: crear una interfaz que mapee message: str, data: T
    async def create_task_list(payload: TaskListCreate):
        try:
            task_created = await task_list_usecase.create_list(payload.to_entity())
            return {"message": "Task List Created", "data": TaskListOut.from_entity(task_created)}
    ...
    async def update_task_list(task_list_id: int, payload: TaskListUpdate):
        try:
            task_updated = await task_list_usecase.update_list(task_list_id, payload.to_entity())
            return {"message": "Task List Updated", "data": TaskListOut.from_entity(task_updated)}
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
