...
def create_task_route(task_usecase: TaskUsecase) -> APIRouter:
    ...
    @router.get("/task-lists/{task_list_id}/tasks", response_model=ApiResponse[TasksList[List[TaskOut]]])
    async def get_tasks_by_list_id(task_list_id: int, request: Request):
        filters = dict(request.query_params)
        tasks, completion_percentage = await task_usecase.list_by_task_list(task_list_id, filters)
        if not tasks:
            raise HTTPException(status_code=404, detail="Tasks Not Found for this List")
        response_data = {"completion_percentage": completion_percentage, "tasks": [TaskOut.from_entity(task) for task in tasks if task]}
        return {"message": "Tasks Obtained", "data": response_data}
    
    @router.post("/task-lists/{task_list_id}/tasks", status_code=status.HTTP_201_CREATED, response_model=ApiResponse[TaskOut])
    async def create_task(task_list_id: int, payload: TaskCreate):
        ...
        try:
            ...
            return {"message": "Task Created", "data": TaskOut.from_entity(task_created)}
